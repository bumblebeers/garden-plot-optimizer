...
(function (root, factory) {
   ...
   * exact anchors. Only pinned cells are marked in `user` (immutable): the
   * remaining selection is free to move in the hill-climb, subject to the
   * selection multiset. `pins` is an array of { sym, r, c } anchors (top-left
  ...
  function buildGrid(selection, pins, policy) {
    ...
    for (const sym of order) {
      ...
      for (let k = 0; k < remaining; k++) {
        ...
        for (let dr = 0; dr < h; dr++) for (let dc = 0; dc < w; dc++) grid[p[0] + dr][p[1] + dc] = sym;
      }
    }
    return { grid, user };
  }
  ...
  /* One random move applied to `g2` in place; returns true if a move was made.
   * The move space always keeps the layout a valid packing and, when `lock` is
   * non-empty, preserves the instance count of every locked symbol — the user's
   * selection acts as a bill of materials (relocatable, never deletable), while
   * autofill material (f1 1x1s / f2 providers) is free. Pinned cells (`user`)
   * never move. */
  function hillMove(g2, user, f1, f2, lock) {
    const insts = buildInstances(g2);
    const instByCell = new Map();
    insts.forEach((it, i) => it.cells.forEach(k => instByCell.set(k, i)));
    const one = [];
    for (let k = 0; k < 81; k++) {
      if (user[k] || g2[Math.floor(k / 9)][k % 9] == null) continue;
      if (insts[instByCell.get(k)].cells.length === 1) one.push(k);
    }
    const selInsts = insts.filter(it => it.cells.length > 1 && lock.has(it.sym) && !it.cells.some(x => user[x]));
    // move shares: swap/retype .40, convert .20, break .15, relocate .25 — the
    // relocate slice is renormalised away when there is no selected multi-cell
    // crop to move, so a fixed budget is not wasted on it
    const wSwap = 0.40, wConv = 0.20, wBreak = 0.15, wReloc = 0.25;
    const tot = selInsts.length ? wSwap + wConv + wBreak + wReloc : wSwap + wConv + wBreak;
    const r = Math.random() * tot;
    if (r < wSwap) { // swap two 1x1 cells (any symbols — preserves counts), or retype an autofill 1x1
      if (one.length < 2) return false;
      const a = one[Math.floor(Math.random() * one.length)];
      const ar = Math.floor(a / 9), ac = a % 9, asym = g2[ar][ac];
      if (f1.includes(asym) && Math.random() < 0.5) {
        const ns = f1[Math.floor(Math.random() * f1.length)];
        if (ns === asym) return false;
        g2[ar][ac] = ns;
      } else {
        const b = one[Math.floor(Math.random() * one.length)];
        if (b === a) return false;
        const br = Math.floor(b / 9), bc = b % 9;
        const t = g2[ar][ac]; g2[ar][ac] = g2[br][bc]; g2[br][bc] = t;
      }
      return true;
    }
    if (r < wSwap + wConv) { // convert a 2x2 all-autofill block into a provider
      const spots = [];
      for (let rr = 0; rr < 8; rr++) for (let cc = 0; cc < 8; cc++) {
        const k0 = rr * 9 + cc, ks = [k0, k0 + 1, k0 + 9, k0 + 10];
        if (ks.some(x => user[x])) continue;
        if (ks.every(x => f1.includes(g2[Math.floor(x / 9)][x % 9]))) spots.push([rr, cc]);
      }
      if (!spots.length || !f2.length) return false;
      const [rr, cc] = spots[Math.floor(Math.random() * spots.length)];
      const sym = f2[Math.floor(Math.random() * f2.length)];
      for (let dr = 0; dr < 2; dr++) for (let dc = 0; dc < 2; dc++) g2[rr + dr][cc + dc] = sym;
      return true;
    }
    if (r < wSwap + wConv + wBreak) { // break a provider back into 1x1s
      const provs = insts.filter(it => it.cells.length === 4 && f2.includes(it.sym) && !it.cells.some(x => user[x]));
      if (!provs.length) return false;
      const it = provs[Math.floor(Math.random() * provs.length)];
      for (const x of it.cells) g2[Math.floor(x / 9)][x % 9] = f1[Math.floor(Math.random() * f1.length)];
      return true;
    }
    // Relocate a selected multi-cell crop: drop it, fill its old footprint with
    // autofill 1x1s, and place it over an all-autofill block of the same size.
    const it = selInsts[Math.floor(Math.random() * selInsts.length)];
    const h = CROP[it.sym].sz[0], w = CROP[it.sym].sz[1];
    const own = new Set(it.cells);
    const targets = [];
    for (let rr = 0; rr <= 9 - h; rr++) for (let cc = 0; cc <= 9 - w; cc++) {
      const cells = [];
      for (let dr = 0; dr < h; dr++) for (let dc = 0; dc < w; dc++) cells.push((rr + dr) * 9 + cc + dc);
      if (cells.some(x => user[x] || own.has(x))) continue;
      // every cell must belong to autofill material: a 1x1 autofill crop, or a
      // single multi-cell autofill crop that exactly covers the block
      const ids = new Set(cells.map(x => instByCell.get(x)));
      let ok = true;
      for (const id of ids) {
        const ci = insts[id];
        if (ci.cells.length === 1 && !lock.has(ci.sym)) continue;
        if (ids.size === 1 && ci.cells.length === h * w && !lock.has(ci.sym) && ci.cells.every(x => cells.includes(x))) continue;
        ok = false; break;
      }
      if (ok) targets.push([rr, cc]);
    }
    if (!targets.length) return false;
    const [tr, tc] = targets[Math.floor(Math.random() * targets.length)];
    for (const x of it.cells) g2[Math.floor(x / 9)][x % 9] = f1[Math.floor(Math.random() * f1.length)];
    for (let dr = 0; dr < h; dr++) for (let dc = 0; dc < w; dc++) g2[tr + dr][tc + dc] = it.sym;
    return true;
  }
  /* Simulated-annealing hill-climb over the full move space. `user` marks
   * pinned (immutable) cells; `lock` is the set of symbols whose instance
   * counts must be preserved (the user's selection). Acceptance is Metropolis
   * with geometric cooling: the search escapes local optima early and refines
   * late, instead of freezing at the first local optimum a greedy climb
   * reaches (the bug #9 reports: the old hill-climb never converged on its own
   * score function, and a plain anneal over the same moves beat it). */
  function hillfill(grid, user, iters, f1, f2, opts, lock) {
    const lockSyms = lock || new Set();
    grid = grid.map(r => r.slice());
    let cur = scoreTotal(grid, opts), best = cur, bg = grid.map(r => r.slice());
    // start warm enough to take moderate steps, cool to near-greedy
    const T0 = Math.max(1e-6, 0.01 * cur);
    const Tmin = 1e-3;
    // T0=0.01·score (colder than 0.02) measured best: it keeps the search near
    // greedy while still escaping local optima, and wins on every scenario
    // tried for both objectives (see issue #8/#9 verification runs).
    const cool = Math.pow(Tmin / T0, 1 / Math.max(1, iters));
    let T = T0;
    for (let it = 0; it < iters; it++) {
      const g2 = grid.map(r => r.slice());
      if (hillMove(g2, user, f1, f2, lockSyms)) {
        const sc = scoreTotal(g2, opts);
        if (sc >= cur || Math.random() < Math.exp((sc - cur) / T)) {
          grid = g2; cur = sc;
          if (sc > best) { best = sc; bg = g2.map(r => r.slice()); }
        }
      }
      T *= cool;
    }
   ...
   * remaining selection around them, then fills and anneals the whole plot.
   * Unpinned selected crops are part of the move space — as a bill of materials
   * they may be relocated, never deleted or retyped away — and a pinned crop's
   * symbol is added to the selection set so the autofill pools never re-insert
   * it. */
  function optimizeSynergy(selection, fill, iters, opts, pins) {
    ...
    // Symbols whose instance count the search must preserve: the user's
    // selection, minus anything the (fallback) autofill pools can re-insert.
    const locked = new Set([...sel].filter(s => !f1.includes(s) && !f2.includes(s)));
    ...
    // Anneal every packing; the iters budget is split across the restarts.
    ...
    for (const r of placed) {
      ...
      if (fill) {
        ...
        const [bg, b] = hillfill(g, user, Math.max(1500, Math.floor(iters / restarts)), f1, f2, opts, locked);
  ...
  return {
    ...
    validateLayout,
    canPlace, localBuff, buildGrid, fillInitial,
    hillfill, optimizeSynergy,
