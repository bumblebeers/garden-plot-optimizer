...
(function (root, factory) {
   ...
   *   income — compare net gold/day per class: what the class's buff adds to the
   *            crop against its price per tile per day. Water Retain / Weed
   *            Block cost gold and add none, so they never win; neither does a
   *            class whose buff the crop already receives from its neighbours.
   *   yield  — only Harvest Boost raises items/day, so Quality / Water / Weed
   *            tie at zero gain and the first of them in the user's priority is
   *            bought (the score's coverage tie-break is what makes that worth
   *            doing — see TIEBREAK). */
  function chooseFertilizer(d, tiles, got, opts) {
    const yieldObj = opts.objective === 'yield';
    // what the crop earns with no fertiliser bought at all
    const noneVal = yieldObj
      ? ((got.H ? d.yldB : d.yld) * d.harv) / d.cycle
      : instanceGross(d, got.H, got.Q, opts);
    // best allowed-and-missing class; equivalent classes keep the earlier one,
    // so the user's buff priority order breaks the tie
    let best = null, bestVal = -Infinity;
    for (const b of effectiveBuffOrder(opts)) {
      if (!FERTBUFFS.includes(b) || !opts.fert[b] || got[b]) continue;
      const v = yieldObj
        ? (((got.H || b === 'H') ? d.yldB : d.yld) * d.harv) / d.cycle
        : instanceGross(d, got.H || b === 'H', got.Q || b === 'Q', opts) - tiles * FERT_PRICE[b];
      if (v > bestVal + 1e-9) { best = b; bestVal = v; }
    }
    if (best === null) return 'None';
    // buy a class only where it pays: strictly under income (a tie leaves the
    // gold with the player), or merely not-worse under yield (the score's
    // coverage tie-break is what makes a convenience buff worth buying there)
    return (yieldObj ? bestVal >= noneVal - 1e-9 : bestVal > noneVal + 1e-9) ? best : 'None';
  }
  ...
  function optimizeSynergy(selection, fill, iters, opts, pins) {
    ...
    // buildGrid is a randomised greedy placer, so a single attempt can fail — or
    // pack differently — by chance even when the selection fits. Attempts are
    // restarts, and "couldn't fit" is only reported once every one has failed.
    // The regular attempts all use the buff-seeking placement (the best starting
    // point for the hill-climb); if every one of them dead-ends, a last few
    // retry with a tighter first-fit and a spread placement before giving up.
    const restarts = 10;
    let bestg = null, bestsc = -Infinity;
    const attempt = (policy) => {
      const r = buildGrid(selection, pins, policy);
      if (!r) return false;
      let g = r.grid;
      const user = r.user;
      if (fill) {
    ...
    let placed = 0;
    for (let k = 0; k < restarts; k++) if (attempt('buff')) placed++;
    if (!placed) for (const policy of ['firstfit', 'firstfit', 'random', 'random']) if (attempt(policy)) break;
